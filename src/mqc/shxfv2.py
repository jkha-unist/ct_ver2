...
from build.el_propagator_xfv2 import el_run
...
#                raise ValueError (f"( {self.md_type}.{call_name()} ) {error_message} ( {error_vars} )")
    ...
    def set_decoherence(self, one_st):
        ...
        for ist in range(self.mol.nst):
            ...
            self.aux.vel_old[ist, :, :] = self.mol.vel[0:self.aux.nat, :]
    ...
    def get_dS(self):
        ...
        if(self.l_pc_w_phase_term):
            ...
            for iat in range(self.aux.nat):
                self.dS_old[:, iat, :] = self.aux.vel_old[:, iat, :] * self.mol.mass[iat]
                self.dS[:, iat, :] = self.aux.vel[:, iat, :] * self.mol.mass[iat]
        ...
        for ist in range(nst):
            for jst in range(nst):
                for iat in range(nat):
                    self.nac_phase[iat, :] += np.imag(self.mol.nac[ist, jst, iat, :] * self.mol.rho[ist, jst]) / self.mol.mass[iat]
                    self.nac_phase_old[iat, :] += np.imag(self.mol.nac_old[ist, jst, iat, :] * self.mol.rho[ist, jst]) / self.mol.mass[iat]
