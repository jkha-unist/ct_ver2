...
class MQC(object):
    ...
    def __init__(self, molecule, thermostat, istate, dt, nsteps, nesteps, \
        ...
        if not (self.propagator in [None, "rk4", "exponential", "sc"]):
    ...
    def run_init(self, qm, mm, output_dir, l_save_qm_log, l_save_mm_log, l_save_scr, restart):
        ...
        if ((self.md_type in ["CT", "CTv2", "CTQD", "CTcrunch", "CTPC", "Eh"]) and (mm != None)):
        ...
        if (self.md_type != "CT" and self.md_type != "CTv2" and self.md_type != "CTQD" and self.md_type != "CTcrunch" and self.md_type != "CTPC"):
...
#        #self.temperature = self.mol.ekin * 2 / float(self.mol.ndof) * au_to_K
    ...
    def print_init(self, qm, mm, restart):
        ...
        if (self.md_type in ["SH", "SHXF", "SHXFQD", "SHXFv2"]):
        ...
        # Print XF variables
        if (self.md_type in ["SHXF", "SHXFQD", "SHXFv2"]):
    ...
    def touch_file(self, unixmd_dir):
        ...
        if (self.md_type != "BOMD"):
            ...
            if (self.verbosity >= 1):
                ...
                #tmp = f'{"#":5s} Time-derivative Density Matrix by ENC: population; see the manual for detail orders'
        ...
        if (self.md_type in ["SHXF","SHXFQD", "SHXFv2", "CT", "CTv2", "CTQD", "CTcrunch", "CTPC"]):
            if (self.verbosity >= 1):
                tmp = f'{"#":5s} Time-derivative Density Matrix by decoherence: population; see the manual for detail orders'
                typewriter(tmp, unixmd_dir, "DOTPOPDEC", "w")

        if (self.md_type in ["IT"]):
            tmp = f'{"#":5s} ENC info, IQMOM, ReQ01, ImQ01 Q00 Q11 F_ENC'
            typewriter(tmp, unixmd_dir, "ENC", "w")
            tmp = f'{"#":5s} ENC2 info, ReQ201_0, ImQ201_0 ReQ201_1 ImQ201_1 Q200_0 Q200_1 Q211_0 Q211_!'
            typewriter(tmp, unixmd_dir, "ENC2", "w")
            tmp = f'{"#":5s} QDCOEF'
            typewriter(tmp, unixmd_dir, "QDCOEF", "w")
            tmp = f'{"#":5s} TD potentials, scalar, vector, egauge, quantum potential'
            typewriter(tmp, unixmd_dir, "TDPES", "w")
            tmp = f'{"#":5s} Time-derivative Density Matrix by 1st order ENC: population; see the manual for detail orders'
            typewriter(tmp, unixmd_dir, "DOTPOPDEC", "w")
            tmp = f'{"#":5s} Time-derivative Density Matrix by 2nd order ENC: population; see the manual for detail orders'
            typewriter(tmp, unixmd_dir, "DOTPOPDEC2", "w")
