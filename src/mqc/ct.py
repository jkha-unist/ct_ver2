...
class CT(MQC):
    ...
    def __init__(self, molecules, thermostat=None, istates=None, dt=0.5, nsteps=1000, nesteps=20, \
        ...
        l_en_cons=False, l_sigma_artifact=True, unit_dt="fs", out_freq=1, verbosity=0):
        ...
        for itraj in range(self.ntrajs):
            ...
            for iat in range(self.nat_qm):
                for idim in range(self.ndim):
                    ...
                    if (self.l_sigma_artifact): # multiply an artifact ntraj(itraj)^0.25 to modulate sigma.
                        self.sigma_lk[itraj, 0, iat, idim] /= np.sqrt(np.sqrt(self.count_ntrajs[itraj, iat, idim]))
