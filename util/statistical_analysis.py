...
def statistical_analysis():
    ...
    averaged_energy(args.ntrajs, digit, nsteps1, args.nstates)
...
def averaged_density_matrix(ntrajs, digit, nsteps, nstates):
    ...
    avg_dot = np.zeros((nstates, nsteps))
    # define variable for count trajectories except halted trajectories
    mtrajs = ntrajs

    for itraj in range(ntrajs):
        path = os.path.join(f"./TRAJ_{itraj + 1:0{digit}d}/md/", "BOPOP")
        path2 = os.path.join(f"./TRAJ_{itraj + 1:0{digit}d}/md/", "DOTPOPDEC")
        ...
        with open(path2, 'r') as f:
            ...
            line2 = f.read()
            lines2 = line2.split()
            lines_list2 = list(map(float, lines2))
        ...
        try:
            ...
            # sum over time-derivative of populations from decoherence term of each states
            avg_dot += np.array([lines2[istate::(nstates + 1)][:nsteps] for istate in range(1, nstates + 1)], dtype=np.float64)
    ...
    avg_data = "".join([("\n" + f"{istep:8d}" + "".join([f"{avg_dot[istate, istep]:15.8f}" \
        for istate in range(nstates)])) for istep in range(nsteps)])
    f3_write += avg_data

    typewriter(f1_write, "AVG_COHRHO")
    typewriter(f2_write, "AVG_POPRHO")
    typewriter(f3_write, "AVG_DOTPOPDEC")
...
def averaged_energy(ntrajs, digit, nsteps, nstates):
    """ Electronic coherence analysis and BO population analysis
        based on the density matrix of each trajectories
    """
    f1_write = ""

    header = "#    Averaged energy: "
    f1_write += header

    # define empty array for summation
    avg_energy = np.zeros((nstates+3, nsteps))
    # define variable for count trajectories except halted trajectories
    mtrajs = ntrajs

    for itraj in range(ntrajs):
        path = os.path.join(f"./TRAJ_{itraj + 1:0{digit}d}/md/", "MDENERGY")
        ...
        try:
            # sum over population of each states
            avg_energy += np.array([lines[istate::(nstates + 4)][:nsteps] for istate in range(1, nstates + 4)], dtype=np.float64)
        except ValueError:
            # exclude halted trajectories from total trajectory number
            mtrajs -= 1
    ...
    avg_data = "".join([("\n" + f"{istep:8d}" + "".join([f"{avg_energy[istate, istep]:15.8f}" \
        for istate in range(nstates+3)])) for istep in range(nsteps)])
    f1_write += avg_data

    typewriter(f1_write, "AVG_ENERGY")
